...
* Updated README
* Added option to add AHK to the system/user PATH environment variable.
* Added option to copy selected/installed EXE to `A_ScriptDir` as `AutoHotkey.exe`
...
Grab the latest copy of AHK v2 (currently beta.3), copy the desired version of `AutoHotkey32/64.exe` into the script dir and rename it to `AHK_Portable_Installer.exe`.  Always run this EXE file to launch the script.
...
Please note that all current settings from the `Options` tab or `Basics` tab are applied on install.  So if you want to make a change to your settings then you need to click the `Install` button again.  This will perform a "reinstall" with the new settings.
...
If you need help post on the forums, or [join #ahk on IRC or visit the AutoHotkey Discord](https://www.autohotkey.com/boards/viewtopic.php?f=76&t=59).  I frequently go on IRC, and I'm usually always connected on Discord.  Send me a PM on the AutoHotkey forums and we can arrange a time to work out some issues.

1) It is NOT recommended to run this script along side a normal installation of AutoHotkey with the setup program, it is however theoretically possible.  But this script will override the proper install with its own settings in the registry.  If you use AHK Portable Installer to install over the top of a legit AHK installation, then you won't be able to uninstall it.  Simply reinstall the legit AHK setup EXE again to be able to uninstall it and remove it from the programs list.

2) When making changes to your settings in the `Basics` tab and the `Options` tab, you will always have to re-apply the installation for those new settings to take effect.  So define your desired settings first, then install/reinstall your chosen version of AutoHotkey.

3) If you move your `base folder`, then you must "re-install" your chosen AutoHotkey `base version`.  Simply click `Install/Select` again to update registry entries.  Keep in mind, if you don't specify a custom `base folder`, and you move the location of AHK Portable Installer, then you are also moving the `base folder`.  The default location of the `base folder` is `%A_ScriptDir%\versions`.

4) Every version folder of AutoHotkey should have its own `help file`, `WindowSpy.ahk`, and `Compiler` folder.

5) You should generally always use the latest Ahk2Exe.  Remember there are 3 different Ahk2Exe's:
...
The latest Ahk2Exe works with both v1 and v2 beta.  See the links in the `Latest Updates` section above.

6) In regards to `A_AhkPath`:
...
7) Running scripts in Fully Portable Mode:
...
8) Wrong PID (Process ID) when using Run/RunWait:
This is something you may encounter if you use `Run(my_script.ahk,,,&pid)`.  Since this program is the actual launcher for the script, this is what is happening:
    ...
    * Launcher is invoked.  <-- this is the PID used
    * Launcher determines which version of AHK to use.
    * Launcher uses `Run()` to launch the script.
...
This program will NOT circumvent User Account Control settings.  If you leave UAC enabled and try to use this program as an installer, then you will get an "Access Denied" error.  You will need to run this program as Administrator.
